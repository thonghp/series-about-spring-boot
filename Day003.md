...
- **`@ManyToMany` ==>** Định nghĩa mối quan hệ many-many giữa 2 entity, tạo mapping giữa 2 bảng.\
    - Mặc định **`fetch = FetchType.LAZY` ==>** Chỉ tải đối tượng chính vào bộ nhớ.
        - Vd: Khi tải cả lớp School chỉ tải 1 mình nó chứ không tải Department, nó chỉ được tải khi được truy cập thông qua School.
    - **`fetch = FetchType.EAGER` ==>** tự động tải các đối tượng liên quan vào bộ nhớ cùng với đối tượng chính.
