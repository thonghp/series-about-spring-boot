 ...
 * trong đó :limit sẽ là số lượng record trả về <=> USERS_PER_PAGE
 *          :offset vị trí bắt đầu để lấy ra <=> pageNum - 1
 * -> pageNum - 1 ở đây vì vị trí đầu tiên của record trong db là 0
 * -> vậy muốn lấy từ id 1 -> 5 <=> offset là 0 và limit 5  
 */
